...
the topic of Irish unity/Irish unification/united Ireland.
The code below is essentially the same as in the
mine_sinnFein_tweets.py file, but querying for 
the aforementioned topics and to extract
...
# Note: please ensure you have numpy install
# prior to install snscrape
...
search_query = 'irish unity OR united Ireland OR irish unification OR #irishunity OR #unitedireland '
...
def get_tweets(search_query, num_of_tweets, from_date, to_date):
    ...
    # Should only occur if issue with dates
...
# 1 hour to finish mining, compared to 1 hour and
# 10 minutes for the sinn fein tweets
scraped_tweets = get_tweets(search_query, num_of_tweets, from_date, to_date)
df = pd.DataFrame(scraped_tweets)
df.to_csv('irish_unity_tweets_RAW.csv', index=False)
